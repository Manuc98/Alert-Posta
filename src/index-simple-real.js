// Alert@Postas - Cloudflare Worker com DADOS REAIS (Versão Simples)
                ...
                if (response.ok && Array.isArray(data)) {
                    ...
                    if (data.length > 0) {
                        ...
                        for (let i = 0; i < Math.min(data.length, 5); i++) {
                            ...
                            html += '</div>';
                            html += '</div>';
                            html += '<div class="text-xl font-bold text-center mb-1">';
                ...
                if (response.ok && Array.isArray(data)) {
                    ...
                    if (data.length > 0) {
                        ...
                        for (let i = 0; i < Math.min(data.length, 5); i++) {
                            ...
                            html += '<div class="flex justify-between items-center mb-2">';
                            html += '<div class="text-sm font-medium text-gray-900 dark:text-white">';
                            html += signal.game || 'Jogo não especificado';
                            html += '</div>';
                            html += '<div class="text-xs px-2 py-1 rounded-full ' + resultClass + '">';
                            html += signal.result || 'Pending';
                            html += '</div>';
                            html += '</div>';
                            html += '<div class="text-sm text-gray-600 dark:text-gray-400 mb-1">';
                            html += signal.prediction || 'Previsão não disponível';
                            html += '</div>';
                            html += '<div class="text-xs text-gray-500 dark:text-gray-400">';
                            html += 'Confiança: ' + (signal.confidence || 0) + '% - ' + (signal.timestamp || 'Agora');
                            html += '</div>';
                            html += '</div>';
                        }
                        container.innerHTML = html;
            ...
            try {
                ...
                button.innerHTML = '<i data-lucide="loader-2" class="h-5 w-5 animate-spin"></i> ' + 
                                  (action === 'start' ? 'Iniciando...' : 
                                   action === 'stop' ? 'Parando...' : 'Reiniciando...');
                ...
                if (response.ok && data.success) {
                    showNotification('Bot ' + (action === 'start' ? 'iniciado' : 
                                             action === 'stop' ? 'parado' : 'reiniciado') + ' com sucesso!', 'success');
                } else {
                    showNotification('Erro ao ' + (action === 'start' ? 'iniciar' : 
                                                 action === 'stop' ? 'parar' : 'reiniciar') + ' bot', 'error');
                }
...
// Função para lidar com sinais - DADOS REAIS DO BOT
async function handleSignalsAPI(request, url, env) {
  try {
    const limit = parseInt(url.searchParams.get('limit')) || 10;
    
    // O bot ainda não está a gerar sinais reais
    // Por isso vamos mostrar uma lista vazia até o bot estar funcional
    const signals = [];
    
    // Log para debug
    console.log('Bot ainda não está a gerar sinais reais. Lista vazia retornada.');
...
// Função para obter estatísticas do site - DADOS REAIS
async function getSiteStats(request, env) {
  // O bot ainda não está totalmente funcional
  // Vamos mostrar dados realistas mas indicar que está em desenvolvimento
  const stats = {
    signals_today: 0, // Nenhum sinal gerado ainda
    accuracy_7d: 0, // Sem dados ainda
    roi_estimated: 0, // Sem dados ainda
    active_model: {
      name: 'Sistema em Desenvolvimento',
      accuracy: 0
    },
    bot_status: {
      status: 'development', // Status real: em desenvolvimento
      uptime: 0,
      modules: [
    ...
    message: 'Sistema em desenvolvimento - Bot ainda não está a gerar sinais reais'
  };
