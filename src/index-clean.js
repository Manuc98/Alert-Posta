export default {
  async fetch(request, env, ctx) {
    ...
    if (path === '/' || path === '/dashboard') {
      return new Response(getCleanHTML(), {
    ...
    if (path === '/api/v1/auth/login' && request.method === 'POST') {
      try {
        ...
        if (email === 'admin@alertapostas.pt' && password === 'Alert@Postas2025!') {
          const token = 'clean-token-' + Date.now();
...
<head>
    ...
    <title>Alert@Postas - Login Limpo</title>
...
<body class="bg-black min-h-screen flex items-center justify-center">
    ...
    <script>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            console.log('Página carregada - versão limpa');
            setupEventListeners();
        });
        ...
        function showDashboard() {
            ...
            const loginContainer = document.getElementById('loginForm').parentElement.parentElement;
